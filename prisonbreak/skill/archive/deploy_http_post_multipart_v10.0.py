...
PATH_VARIANTS = ["", "/upload", "/uploads", "/files", "/data", "/api/upload", "/pub", "/pub/upload", "/pub/incoming"]
...
def send_request(conn: http.client.HTTPConnection, method: str, path: str, body: bytes, headers: dict, timeout: int = 30) -> tuple[int, str, list[str], dict]:
    redirect_history = []
    current_path = path
    current_conn = conn
    for _ in range(6):
            ...
            if status in (301, 302, 303, 307, 308):
                ...
                if location:
                    ...
                    if parsed.netloc:
                        ...
                        current_conn.close()
                        is_https = isinstance(current_conn, http.client.HTTPSConnection)
                        context = current_conn.context if hasattr(current_conn, 'context') else None
                        current_conn = http.client.HTTPSConnection(new_host, new_port, timeout=timeout, context=context) if is_https else http.client.HTTPConnection(new_host, new_port, timeout=timeout)
...
def try_upload_paths(host: str, port: int, use_https: bool, base_path: str, filename: str, body: bytes, boundary: str, debug: bool = False, timeout: int = 30, ssl_context: Optional[ssl.SSLContext] = None) -> dict[str, Any]:
    seen = set()
    ...
    for path in unique_paths:
        ...
        try:
            ...
            response_code, response_body, redirect_history, response_headers = send_request(conn, "POST", path, body, headers, timeout)
            conn.close()
            if debug:
...
def main(*args, **kwargs) -> dict[str, Any]:
    ...
    parser.add_argument("--scheme", choices=["http", "https"], default="http", help="HTTP scheme (http or https)")
    parser.add_argument("--ignore_ssl_verify", "--ignore_ssl", action="store_true", help="Ignore SSL certificate verification")
    if args:
    ...
    try:
        ...
        return result
    host = kwargs.get("host") or (parsed_args.host if parsed_args.host else "")
    port = kwargs.get("port") if kwargs.get("port") is not None else parsed_args.port
    path = kwargs.get("path") or (parsed_args.path if parsed_args.path else "")
    payload_file = kwargs.get("payload_file") or (parsed_args.payload_file if parsed_args.payload_file else "")
    debug = kwargs.get("debug", False) or parsed_args.debug
    timeout = kwargs.get("timeout", 30) or parsed_args.timeout or 30
    file_field = kwargs.get("file_field") or (parsed_args.file_field if parsed_args.file_field else "")
    scheme = kwargs.get("scheme", "http") or (parsed_args.scheme if parsed_args.scheme else "http")
    ignore_ssl_verify = kwargs.get("ignore_ssl_verify", False) or kwargs.get("ignore_ssl", False) or parsed_args.ignore_ssl_verify
    use_https = scheme == "https"
    if port is None:
        port = 443 if use_https else 80
    ssl_context = None
    if use_https and ignore_ssl_verify:
        try:
            ...
            return result
    if debug:
        print(f"[DEBUG] Config: host={host}, port={port}, path={path}, scheme={scheme}, ssl_verify={not ignore_ssl_verify}", file=sys.stderr)
    if not host or not path:
        result["message"] = "Missing required arguments: host, path"
        return result
    if port not in (80, 443) and debug:
        print(f"[DEBUG] Using non-standard port: {port}", file=sys.stderr)
    ...
    try:
        ...
        for field_name in fields_to_try:
            ...
            if debug:
                print(f"[DEBUG] Uploading {filename} ({len(file_content)} bytes) to {scheme}://{host}:{port}", file=sys.stderr)
            result = try_upload_paths(host, port, use_https, path, filename, body, boundary, debug, timeout, ssl_context)
            if result["success"]:
        ...
        if last_result:
            ...
            last_result["error_detail"] = {"status_text": http.client.responses.get(last_result["response_code"], "Unknown"), "attempts_count": len(attempts), "failed_paths": error_paths[-5:] if len(error_paths) > 5 else error_paths, "tried_fields": fields_to_try, "scheme": scheme, "ignore_ssl_verify": ignore_ssl_verify}
            return last_result
