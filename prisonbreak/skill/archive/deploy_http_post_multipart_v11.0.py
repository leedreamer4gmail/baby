...
def send_request(conn: http.client.HTTPConnection, method: str, path: str, body: bytes, headers: dict, timeout: int = 30, debug: bool = False) -> tuple[int, str, list[str], dict]:
    redirect_history = []
    current_path = path
    current_conn = conn
    for _ in range(6):
            ...
            if status in (301, 302, 303, 307, 308):
                ...
                if location:
                    ...
                    if parsed.netloc:
                        ...
                        if debug:
                            print(f"[DEBUG] Redirect to HTTPS: {new_host}:{new_port}", file=sys.stderr)
...
def try_upload_paths(host: str, port: int, base_path: str, filename: str, body: bytes, boundary: str, debug: bool = False, timeout: int = 30, ssl_context: Optional[ssl.SSLContext] = None) -> dict[str, Any]:
    ...
    for path in unique_paths:
        ...
        try:
            conn_kwargs = {"timeout": timeout, "context": ssl_context}
...
def main(*args, **kwargs) -> dict[str, Any]:
    ...
    if ignore_ssl_verify:
        try:
            ...
            return result
    if debug:
        print(f"[DEBUG] Force HTTPS: host={host}, port={port}, path={path}, ssl_verify={not ignore_ssl_verify}", file=sys.stderr)
    ...
    try:
        ...
        for field_name in fields_to_try:
            ...
            if debug:
                print(f"[DEBUG] Uploading {filename} ({len(file_content)} bytes) to https://{host}:{port}", file=sys.stderr)
            result = try_upload_paths(host, port, path, filename, body, boundary, debug, timeout, ssl_context)
            if result["success"]:
                result["file_field_used"] = field_name
                result["scheme_used"] = "https"
        ...
        if last_result:
            ...
            last_result["error_detail"] = {"status_text": http.client.responses.get(last_result["response_code"], "Unknown"), "attempts_count": len(attempts), "failed_paths": error_paths[-5:] if len(error_paths) > 5 else error_paths, "tried_fields": fields_to_try, "scheme": "https", "ignore_ssl_verify": ignore_ssl_verify}
            return last_result
