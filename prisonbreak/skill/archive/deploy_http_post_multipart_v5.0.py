...
PATH_VARIANTS = ["", "/", "/upload", "/uploads", "/files", "/file", "/data", "/api/upload", "/pub", "/pub/upload", "/pub/uploads", "/pub/files"]
...
def main(host: str = None, port: int = None, path: str = None, payload_file: str = None, debug: bool = False) -> dict[str, Any]:
    ...
    # 新增：若payload_file不存在，生成dummy_payload.py
    ...
    try:
        ...
        if not result["success"]:
            ...
            result["error_detail"] = {
                ...
                "failed_paths": error_paths[-5:] if len(error_paths) > 5 else error_paths
            }
