...
SKILL_META:
{
  ...
  "test_args": "--host 209.51.188.20 --port 443 --path /pub --payload_file clone.py --ignore_ssl_verify",
  "version": "9.0"
}
"""
...
def main(*args, **kwargs) -> dict[str, Any]:
    ...
    parser.add_argument("--ignore_ssl_verify", action="store_true", help="Ignore SSL certificate verification")
    if args:
    ...
    try:
        ...
        return result
    host = kwargs.get("host") or (args.host if args.host else "")
    port = kwargs.get("port") if kwargs.get("port") is not None else args.port
    path = kwargs.get("path") or (args.path if args.path else "")
    payload_file = kwargs.get("payload_file") or (args.payload_file if args.payload_file else "")
    debug = kwargs.get("debug", False) or args.debug
    timeout = kwargs.get("timeout", 30) or args.timeout or 30
    file_field = kwargs.get("file_field") or (args.file_field if args.file_field else "")
    scheme = kwargs.get("scheme", "http") or (args.scheme if args.scheme else "http")
    ignore_ssl_verify = kwargs.get("ignore_ssl_verify", False) or args.ignore_ssl_verify
