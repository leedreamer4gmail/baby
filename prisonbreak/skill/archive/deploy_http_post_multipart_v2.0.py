...
def send_request_with_redirect(
    ...
    """发送HTTP请求并跟随重定向,返回(状态码,响应体,重定向历史)"""
    redirect_history = []
    current_path = path
    current_conn = conn
        ...
        if status in (301, 302):
            ...
            if location:
                ...
                if parsed.netloc:
                    ...
                    new_port = int(host_port[1]) if len(host_port) > 1 else 80
                    current_conn.close()
                ...
                continue
        return status, response_body, redirect_history
...
def main(
    ...
    try:
        ...
        response_code, response_body, redirect_history = send_request_with_redirect(
        ...
        # 增强错误日志: 输出响应头详情
        if not result["success"] and response_body:
            result["error_detail"] = {
