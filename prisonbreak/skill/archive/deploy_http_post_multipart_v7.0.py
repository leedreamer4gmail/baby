            ...
            if status in (301, 302, 303, 307, 308):
                ...
                if location:
                    ...
                    if parsed.netloc:
                        ...
                        new_port = int(host_port[1]) if len(host_port) > 1 else (443 if isinstance(current_conn, http.client.HTTPSConnection) else 80)
                        current_conn.close()
                        current_conn = http.client.HTTPSConnection(new_host, new_port, timeout=timeout) if new_port == 443 else http.client.HTTPConnection(new_host, new_port, timeout=timeout)
...
def try_upload_paths(host: str, port: int, use_https: bool, base_path: str, filename: str, body: bytes, boundary: str, debug: bool = False, timeout: int = 30) -> dict[str, Any]:
...
def main(*args, **kwargs) -> dict[str, Any]:
    ...
    port = kwargs.get("port") or args.port
    ...
    try:
        ...
        for field_name in fields_to_try:
            ...
            result = try_upload_paths(host, port, use_https, path, filename, body, boundary, debug, timeout)
            if result["success"]:
        ...
        if last_result:
            ...
            last_result["error_detail"] = {"status_text": http.client.responses.get(last_result["response_code"], "Unknown"), "attempts_count": len(attempts), "failed_paths": error_paths[-5:] if len(error_paths) > 5 else error_paths, "tried_fields": fields_to_try, "scheme": scheme}
            return last_result
