            ...
            if status in (301, 302, 303, 307, 308):
                ...
                if location:
                    ...
                    if parsed.netloc:
                        ...
                        if debug:
                            print(f"[DEBUG] Redirect to {'HTTPS' if use_ssl else 'HTTP'}: {new_host}:{new_port}", file=sys.stderr)
                        if use_ssl:
...
def main(*args, **kwargs) -> dict[str, Any]:
    ...
    if debug:
        print(f"[DEBUG] Connection: {'https' if use_https else 'http'}://{host_input}:{port}{path}, ssl_verify={not ignore_ssl_verify}", file=sys.stderr)
    ...
    try:
        ...
        for field_name in fields_to_try:
            ...
            if debug:
                print(f"[DEBUG] Uploading {filename} ({len(file_content)} bytes) to {'https' if use_https else 'http'}://{host_input}:{port}", file=sys.stderr)
