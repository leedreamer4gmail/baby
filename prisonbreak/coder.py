...
版本: 2.0
职责: 根据 brain 的需求 prompt 调用 MiniMax 生成工具代码（带 TOOL_META 头）
"""
...
def _validate_tool_meta(code: str, name: str) -> tuple[bool, str]:
    """校验生成代码是否包含合格的 TOOL_META 头部（不阻断写入，仅预警）

    Returns:
        (valid, message)
    """
    def _warn(msg: str) -> tuple[bool, str]:
        log_fail("coder.py", "_validate_tool_meta", msg, {"name": name})
        print(f"[coder] ⚠️ {name}: {msg}，将由结构测试负责修复", flush=True)
        return False, msg

    if "TOOL_META:" not in code:
        return _warn("缺少 TOOL_META 块")
    if '"version"' not in code:
        return _warn("TOOL_META 缺少 version 字段")
    return True, "TOOL_META OK"
...
def write_tool(name: str, prompt: str) -> Path | None:
    ...
    messages: list[dict[str, str]] = [
        {
            ...
            "content": (
                ...
                "每个工具文件必须严格包含 TOOL_META 头部（写在模块 docstring 里），"
                "格式见用户需求中的模板。缺少 TOOL_META 视为不合格。"
            ),
        },
    ...
    _validate_tool_meta(code, name)  # 校验但不阻断，让结构测试负责修复
...
def _build_fix_prompt(prompt: str, current_code: str, error: str) -> str:
    ...
    return (
        ...
        f"# 修复要求\n"
        f"- 修复上述错误\n- 保持原有功能不变\n- 代码 < 150 行\n"
        f"- 必须保留 TOOL_META 头部（在模块 docstring 中）\n"
    ...
    messages: list[dict[str, str]] = [
        {
            ...
            "content": (
                ...
                "保留 TOOL_META 头部（更新 version），保持 main() 接口不变。\n"
                "只输出代码，不要解释。"
            ),
        },
